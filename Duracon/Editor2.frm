...
Begin VB.Form frm_ca_board2 
   ...
   Begin VB.Frame Title_3 
      ...
      Begin VB.ComboBox Prfsize 
         ...
         EndProperty
         Height          =   315
         ItemData        =   "Editor2.frx":324A
         Left            =   4470
         List            =   "Editor2.frx":3266
...
With doc_props(doc)
    If Age.Text <> "" Then
        ...
        If IsNumeric(Age.Text) Then
            ...
            If Err.Number <> 0 Then
                If Err.Number = 6 Then
                    MsgBox ("Numeric value too large")
                    Age.SetFocus
                    store_data = False
                    Exit Function
                Else
                    MsgBox ("Please report this bug: " & "Error " & Err.Number & " ")
                    Age.SetFocus
                    store_data = False
                    Exit Function
                End If
    ...
    Else
        If Prfpred_2.Value = True Then
            MsgBox ("Only Positive numeric Values allowed!")
            Age.SetFocus
            store_data = False
            Exit Function
        End If
...
With doc_props(doc)
    ...
    Next j
    If .prfsz < 5 Or .prfvdp(0) = 0 Or .prfvcs(0) = 0 Then
        If msg Then MsgBox ("Fill in all cells!"), 48, "Data missing ERROR"
        store_data = False
    Else
        For i = 0 To (.prfsz - 2)
            If (.prfvdp(i) >= .prfvdp(i + 1)) Then
                If msg Then MsgBox ("Profile depth in increasing order!"), 48, "Data ERROR"
                store_data = False
                Exit Function
            End If
        Next i
