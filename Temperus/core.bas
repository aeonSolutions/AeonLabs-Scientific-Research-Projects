...
Public Sub calculus()
    ...
    For j = 1 To FState(i).Conta - 1
       ...
       material(j).l = CDbl(document(i).lista.Text) * 0.001
       document(i).lista.col = 2
       material(j).area = CDbl(document(i).lista.Text) * 0.000001
       document(i).lista.col = 3
       material(j).k = CDbl(document(i).lista.Text)
       document(i).lista.col = 4
       material(j).q0 = CDbl(document(i).lista.Text)
 ...
 '       .Text = v(i)
  ...
  ' End With
    ...
    With material(1)
        f(1) = -1 * .e * .area * .alfa * Abs(.t1 - gauss_result(1))
        f(2) = 1 * .e * .area * .alfa * Abs(.t1 - gauss_result(1))
        m(2) = 1
        m(1) = .n
    End With
    m(1) = 0
